        ...
        <ul className="flexBetween mx-auto text-center">
          {NAV_LINKS.map((link) => (
            <Link
              ...
              to={link.href}
              key={link.title}
              className="lg:regular-24 regular-16 py-2 lg:px-5 px-2 text-primary-600 flexCenter cursor-pointer transition-all hover:text-second-900">
              {link.label}
            </Link>
          ))}
          {authUser ? (
            <li className="px-5 relative cursor-pointer">
              {gender == "male" ? (
                ...
                <img src={female} width={50} onClick={handlerUser} alt="" />
              ) : (
                <FaUserCircle className="text-[30px]" onClick={handlerUser} />
              )}

              {OpenUser && (
                <ul
                  id="userSign"
                  className="absolute z-50 bg-second-100 top-[62px] right-[-35px] pt-5 px-5 text-black text-[18px] rounded bg-back">
                  <li className="py-2 flexCenter gap-x-5 text-gray-500">
                    Hello:<span className="text-primary-400">{user.name}</span>
                  </li>
                  <li className="py-2">
