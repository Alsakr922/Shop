...
import { FaCartPlus, FaCheck, FaHeart, FaStar, FaStarHalf } from "react-icons/fa"
      ...
      <div className="container m-auto bg-slate-100">
        ...
        <div className="grid lg:grid-cols-2 grid-cols-1">
          ...
          <div>
                  ...
                  <p className="text-xl">Processor: <span className="text-gray-700">11 Generation Intel® Core™ i3-1115G4 (up to 4.1 GHz with Intel® Turbo Boost Technology, 6 MB L3 cache, 2 cores, 4 threads)</span></p>
                  <p className="text-xl">Ram: <span className="text-gray-700">4GB</span></p>
                  <p className="text-xl">Hard Disk: <span className="text-gray-700">1TB</span></p>
                    <p className="text-xl">Graphics Card: <span className="text-gray-700">Intel UHD Graphics</span></p>
                    <div className="flex items-center gap-x-5 mb-2 text-xl font-medium text-gray-900 dark:text-white"
                    >
                    {product.colors.map((color , index) => {
                      return (
                        <label className="h-8 w-8 rounded-full relative cursor-pointer" style={{background:color}}   key={index} >
                          ...
                          {Color === color? <FaCheck className="text-[34px] text-primary-900" /> : ''}
                        </label>
                        )
                      })}
                  </div>
